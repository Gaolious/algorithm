...
struct FloydWarshal {
    ...
    void updateRoute(int from, int to, int dist) {
        D[from][to] = min(D[from][to], dist);
    }

    void getRoute(int from, int to) {
        if ( P[from][to] >= 0 ) {
            getRoute(from, P[from][to]);
            route[ routeIndex ++ ] = P[from][to];
            getRoute(P[from][to], to);
        }
    }
    void sol() {
        int from, to, mid;
    ...
    void printRoute() {
        ...
        for ( from = 1 ; from <= N ; from ++ ) {
            for (to = 1; to <= N; to++) {
                routeIndex = 0;
                if ( from != to && D[from][to] < INF ) {
                    route[routeIndex++] = from;
                    getRoute(from, to);
                    route[routeIndex++] = to;
