...
void _update_lazy(int idx, int s, int e) {
    if ( Tree[idx].lazy_add == 0 && Tree[idx].lazy_set <= -INF )
        return ;

    if ( Tree[idx].lazy_set <= -INF ) {
...
void update_div(int idx, int s, int e, int l, int r, ll d ) {
    _update_lazy(idx, s, e);
    if (s > r || e < l ) return;
    if (l <= s && e <= r )
    {
        ll fs_min = floor((double)Tree[idx].min_val/d);
        ll fs_max = floor((double)Tree[idx].max_val/d);
    ...
    int m ;
    m = ( s + e ) / 2 ;
    update_div(idx * 2, s, m, l, r, d);
    update_div(idx * 2 + 1, m + 1, e, l, r, d);
    Tree[idx] = merge(Tree[idx*2], Tree[idx*2+1]);
}
...
Node init(int idx, int s, int e) {
    if ( s == e ) {
        return Tree[idx] = Node{A[s], A[s], A[s], 0, -INF};
    }
    int m = ( s + e ) / 2 ;
