...
struct QUERY {
    ...
    bool operator < (QUERY &o) const {
        ...
        int a=s>>9, b=o.s>>9;
        return a<b || a==b && (a&1 ? e>o.e : e<o.e); // <--- ???
...
long long Count[1'000'000 + 1]={0,};

void update(int idx, long long &currAnswer, bool plus ) {
    long long int &cnt = Count[ A[idx] ];
    long long int prev = cnt, next ;
    ...
    currAnswer -= prev * prev * A[idx] ;
    currAnswer += next * next * A[idx];
}
