...
struct EulerTechnique {
    vector<vector<int>> childs;
    vector<int> in, out;
    int idx;
    int N ;
    EulerTechnique(int n) {
        N = n;
        in.resize(n+1);
        out.resize(n+1);
    ...
    void addConn(int parent, int child) {
        childs[parent].push_back(child);
    }
    void dfs(int n) {
        in[n] = ++ idx;
        for (auto ch: childs[n])
            dfs(ch);
        out[n] = idx;
    }
};
...
void process() {
    ...
    EulerTechnique et(N);
    Fenwick fw2Top(N+1), fw2Bottom(N+1);

    for ( i = 1 ; i <= N ; i ++ ) {
        cin >> n ;
        if ( n > 0 ) et.addConn(n, i);
    }
    et.dfs(1);
    ...
    while (M--) {
        ...
        if ( n == 1 ) {
            cin >> a >> b ;
            if ( isTop2Bottom ) {
                fw2Bottom.update( et.in[a], b);
                fw2Bottom.update( et.out[a]+1, -b);
            }
            else {
                fw2Top.update( et.in[a], b);
            }
        }
        else if ( n == 2 ) {
            cin >> a ;
            cout << fw2Bottom.sum(et.in[a]) + fw2Top.sum(et.out[a]) - fw2Top.sum(et.in[a]-1) << '\n';
        }
        else if ( n == 3 ) {
            isTop2Bottom = !isTop2Bottom;
        }
    }
}
