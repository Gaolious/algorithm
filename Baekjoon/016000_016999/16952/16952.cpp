...
int main()
{
        ...
        for ( y1 = 0 ; y1 < N ; y1 ++ )
            for ( x1 = 0 ; x1 < N ; x1 ++ )
                for ( t2 = 0 ; t2 < 3 ; t2 ++ )
                    for ( y2 = 0 ; y2 < N ; y2 ++ )
                        for ( x2 = 0 ; x2 < N ; x2 ++ )
                            D[IDX(y1, x1, t1)][ IDX(y2, x2, t2) ] = {INF, INF};

    for ( t1 = 0 ; t1 < 3 ; t1 ++ )
        for ( y1 = 0 ; y1 < N ; y1 ++ )
            for ( x1 = 0 ; x1 < N ; x1 ++ )
                for ( t2 = 0 ; t2 < 3 ; t2 ++ )
                    D[IDX(y1, x1, t1)][ IDX(y1, x1, t2) ] = {t1!=t2, t1!=t2};
            ...
            for ( x1 = 0 ; x1 < N ; x1 ++ )
                for ( d = 0 ; d < lenDxy[t1] ; d ++ ) {
                    ...
                    while ( true ) {
                        ...
                        D[ IDX(y1, x1, t1) ][ IDX(ty, tx, t1) ] = {dist, 0} ;
                        if ( t1 == 2 ) dist ++;
                    }
                }
    ...
    for ( i = 2 ; i <= N*N ; i ++ ) {
        y2 = Index[i] / N ;
        x2 = Index[i] % N ;
        for ( j = 0 ; j < 3 ; j ++ ) ans[i%2][j] = {INF, INF};
