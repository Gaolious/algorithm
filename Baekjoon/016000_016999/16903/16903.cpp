...
const int MaxBit = 30;
void addData(Data *root, int n) {
	int idx;
	for ( int bit = MaxBit; ; bit -- ) {
		idx = (n>>bit)&1;
		if ( !root->BIT[ idx ] ) root->BIT[idx] = newData();
		root->BIT[idx]->cnt++;
		root = root->BIT[idx];
		if ( !bit ) break;
	}
...
void delData(Data *root, int n, int bit = MaxBit) {
	int idx;
	for ( int bit = MaxBit; ; bit -- ) {
		idx = (n>>bit)&1;
		root->BIT[idx]->cnt--;
...
int query(Data *root, int n) {
	int idx;
	for ( int bit = MaxBit; root ; bit -- ) {
		idx = (n>>bit)&1;
		if ( root->BIT[ 1 - idx ] && root->BIT[ 1 - idx ]->cnt > 0 )
			root = root->BIT[1 - idx];
		else if ( root->BIT[ idx ] && root->BIT[ idx ]->cnt > 0 )
			root = root->BIT[idx];
		else
			root = nullptr;
		assert(root);
...
void process() {
	...
	while (N--) {
		cin >> c >> x ;
		if ( c == 1 ) addData(root, x);
		else if ( c == 2 ) delData(root, x);
		else {
			cout << query(root, x) << '\n';
		}
	}
