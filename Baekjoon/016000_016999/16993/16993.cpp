...
struct SegmentTree {
	struct Node {
		ll LmaxSum, RmaxSum, AllSum, maxSum;
	};
	Node Default = {-INF,-INF, -INF, -INF};
	int base{};
	vector<Node> T{};
	int N{}, H{} ;
	void init(int n) {
	...
	ll query(int l, int r) {
		...
		for ( l += base, r+=base ; l <= r ; l/=2, r/=2 ) {
			if ( l % 2 == 1 ) lret = Merge(lret, T[l++]);
			if ( r % 2 == 0 ) rret = Merge(T[r--], rret);

		}
		return Merge(lret, rret).maxSum ;
	}
};
void process() {
	int N, i, n, l, r;
	...
	for ( i = 0 ; i < N ; i ++ ) {
		cin >> n;
		st.T[ st.base + i ].AllSum = n;
		st.T[ st.base + i ].LmaxSum = n;
		st.T[ st.base + i ].RmaxSum = n;
		st.T[ st.base + i ].maxSum = n;
	}
	st.build();
	int M ;
	cin >> M ;
