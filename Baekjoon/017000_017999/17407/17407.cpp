...
struct SegmentTree {
	...
	Node Merge(Node l, Node r) {
		Node ret{} ;
		ll newPair = min(l.L, r.R);
		ret.L = l.L + r.L - newPair;
		ret.R = l.R + r.R - newPair;
		ret.Pair = l.Pair + r.Pair + newPair;
		return ret;
	}
	void build() {
	...
	void Update(int idx, Node node) {
		for ( T[idx+=base] = node, idx/=2 ; idx >= 1 ; idx/=2 )
...
void process() {
	...
	for ( i = 0 ; i < N ; i ++ ) {
		st.T[ st.base + i ].L = s[i] == '(';
		st.T[ st.base + i ].R = s[i] == ')';
		st.T[ st.base + i ].Pair = 0;
	}
	st.build();
