#include <bits/stdc++.h>
using namespace std;
typedef long long int ll;
using Pt = pair<int, int>;

int Y, X;
int dy[] = { 0, 1, 0, -1};
int dx[] = { 1, 0, -1, 0};

void init(){
}
pair<Pt, int> GetPos(vector<string> &A) {
    int i, j;
    for (i = 0; i < Y; i++) {
        for (j = 0; j < X; j++) {
            if (A[i][j] == '-') {
                if (j > 0) {
                    if (A[i][j - 1] == '/') return {{i, j}, 1};
                    if (A[i][j - 1] == '\\') return {{i, j}, 3};
                }
            } else if (A[i][j] == '|') {
                if (i > 0) {
                    if (A[i - 1][j] == '/') return {{i, j}, 0};
                    if (A[i - 1][j] == '\\') return {{i, j}, 2};
                }
            }
        }
    }
    return { {0,0}, -1};
}

struct Data {
    Pt p;
    int dir;
    int step;
};

bool validPos(vector<string> &A, int ty, int tx) {
    if ( ty < 0 || ty >= Y || tx < 0 || tx >= X )
        return false;
    return true;
}

void process(int Case) {
    int i, d, ty, tx;
    int C;
    cin >> Y >> X >> C;
    vector<string> A(Y);
    for (auto &s: A) cin >> s ;
    auto [sp, sd] = GetPos(A);
    A[sp.first][sp.second] = '.';
    A[sp.first + dy[(sd+3)%4]][sp.second + dx[(sd+3)%4]] = '.';
    A[sp.first + dy[(sd+1)%4]][sp.second + dx[(sd+1)%4]] = '.';

    string s;
    while ( C -- ) {
        cin >> s ;
        ty = sp.first, tx = sp.second,  d = sd;
        if ( s[0] == 'L') d  = (d+3)%4;
        else if ( s[0] == 'R' ) d  = (d+1)%4;
        else if ( s[0] == 'A' ) d  = (d+2)%4;
        else if ( s[0] == 'F' ) ty += dy[d], tx += dx[d];
        cin >> s ;

        if ( !validPos(A, ty,tx) ) continue;
        if ( !validPos(A, ty + dy[(d+3)%4],tx + dx[(d+3)%4]) ) continue;
        if ( !validPos(A, ty + dy[(d+1)%4],tx + dx[(d+1)%4]) ) continue;

        sp = {ty, tx};
        sd = d;
    }

    if ( sd == 1 || sd == 3 ) {
        A[sp.first][sp.second] = '-';
        if ( sd == 1 ) {
            A[sp.first][sp.second - 1] = '/';
            A[sp.first][sp.second + 1] = '\\';
        }
        else{
            A[sp.first][sp.second-1] = '\\';
            A[sp.first][sp.second+1] = '/';
        }
    }
    else {
        A[sp.first][sp.second] = '|';
        if ( sd == 0 ) {
            A[sp.first - 1][sp.second] = '/';
            A[sp.first + 1][sp.second] = '\\';
        }
        else{
            A[sp.first - 1][sp.second] = '\\';
            A[sp.first + 1][sp.second] = '/';
        }
    }
    for (auto &s : A)
        cout << s << '\n';
}

int main()
{
#ifdef AJAVA_DEBUG
freopen("input.txt", "rt", stdin);freopen("output.txt", "wt", stdout);
#endif
    cin.tie(nullptr)->sync_with_stdio(false);
	int T=1;
	init();
	// cin >> T;
	for ( int i=1 ; i <= T ; i ++ )
	    process(i);
    return 0;
}
