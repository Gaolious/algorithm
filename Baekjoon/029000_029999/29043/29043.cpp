...
void process() {
    // 드라큘라는 메이비스에게 자연수 $A$와 $B$를 말해줍니다,
    // 그 후 메이비스는 $B$를 초과하지 않는 자연수 $x$를 찾아야 하는데,
    // $lcm(A, x)$가 $B$를 초과하지 않는 모든 자연수 중에서 최대가 되어야 합니다.
    // 드라큘라는 딸과 $T$ 라운드를 하고 싶어합니다.
    // find max X for LCM ( A, x ) <= B
    int T;
    ll A, B;
    cin >> T;
    while ( T--){
        cin >> A >> B;
        ll bestLCM = -1;
        for (ll i = B ; i >= 1 && A*i > bestLCM ; i -- ) {
            bestLCM = max( lcm(A, i), bestLCM);
        }
        cout << bestLCM << '\n';
    }
}
