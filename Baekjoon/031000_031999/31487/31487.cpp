...
struct Data {
    ...
    void updateScore() {
        sort(trampoline, trampoline+6);
        totalScore = solvedProblems * 10 ;
        for ( int i = 1 ; i < 5 ; i ++ ) totalScore += trampoline[ i ];
    }
    bool operator <(Data &o) {
        return totalScore == o.totalScore ? idx < o.idx : totalScore > o.totalScore;
    }
};
...
void process() {
    ...
    for (auto &d: A) {
        d.idx = idx++;
        cin >> d.name >> d.solvedProblems;
        for ( i = 0 ; i < 6 ; i ++ ) cin >> d.trampoline[i];
        d.updateScore();
    }
    sort(A.begin(), A.end());
    ...
    for ( i = 0 ; i < N && i < 1000 ; ) {
        A[i].rank = rank;
        for ( ++i, rank++; i < N && i < 1000 && A[i].totalScore == A[i-1].totalScore ; i++, rank++ )
            A[i].rank = A[i-1].rank;

    }

    for ( i = 0 ; i < N && A[i].rank <= 3 && i < 1000 ; i ++ ) {
        cout << A[i].name << ' ' << A[i].totalScore << '\n';
    }
}
