...
struct Data {
    ...
    void updateBingo () {
        ...
        bool f;
        for ( y = 0 ; y < 5 ; y ++ ) {
            for (f = true, x = 0; x < 5 && f ; x++) f = f && B[y][x] ;
            if (f) { bingo = true; return; }
            for (f = true, x = 0; x < 5 && f ; x++) f = f && B[x][y] ;
            if (f) { bingo = true; return; }
        }
        for (f = true, x = 0; x < 5 && f ; x++) f = f && B[x][x] ;
        if (f) { bingo = true; return; }

        for (f = true, x = 0; x < 5 && f ; x++) f = f && B[4-x][x] ;
        if (f) { bingo = true; return; }
    }
};
void process() {
    ...
    while ( M -- ) {
        ...
        for (auto &d: A)
            if ( d.M[n].first > -1 )
                d.B[ d.M[n].first ][ d.M[n].second ] = true;
    }
    int ans = 0;
    for (auto &d: A) {
        d.updateBingo();
