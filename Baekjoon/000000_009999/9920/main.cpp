...
typedef enum { ALL_1, ALL_0, DIFF } KIND;
struct RET {
    KIND kind ;
    int cnt ;
    bool operator == (RET r) const {
        return ( this->kind == r.kind );
    }
};

RET F( int sy, int sx, int w) {
    RET ret ;
    ...
    if ( w == 1 ) {
        ...
        RET r2 = F(sy+m, sx, m);
        RET r3 = F(sy, sx+m, m);
        RET r4 = F(sy+m, sx+m, m);

        if ( r1 == r2 && r2 == r3 && r3 == r4 ) {
            int cnt = 0 ;
            if ( r1.kind == ALL_0 || r1.kind == ALL_1 )
                cnt = 2;
            else
                cnt = r1.cnt + r2.cnt + r3.cnt + r4.cnt + 1;
            ret = RET{ r1.kind, cnt };
        }
        else {
            int cnt ;
