...
void F(FIO &fio, int lenL, int lenC, int vowels, int consonants) {
    if ( lenL >= L ) {
        if ( vowels >= 1 && consonants >= 2 ) {
            fio.write_Str(Output);
            fio.write_Char('\n');
        }
        return ;
    }
    if ( C - lenC < L - lenL ) return ;
    if ( L - lenL < 2 - consonants ) return ;
    if ( L - lenL < 1 - vowels ) return ;

    int nVowel = is_vowel(A[lenC]) ? 1 : 0 ;
    int nConsonant = 1 - nVowel ;

    Output[lenL] = A[lenC];

    F(fio, lenL+1, lenC+1, vowels + nVowel, consonants + nConsonant);
    F(fio, lenL, lenC+1, vowels, consonants);
}
void process() {
    FIO fio;
    int i ;
