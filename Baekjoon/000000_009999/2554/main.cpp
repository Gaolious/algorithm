...
int Factorial[] = { 1, 1, 2, 6, 4, 2, 2, 4, 2, 8, 8};
int twoPower[] = {6, 2, 4, 8};
int last_non_zero_digit(bigint n) {
    if ( n <= 10 )
        return Factorial[n.longValue()];
    // 2^A x A! * B!
    auto t = divmod(n, 5);

    int bFactorial = Factorial[(int)t.second.longValue()];
    int p2 = t.first <= 1 ? 1 : twoPower[ t.first % 4];
    int k, ret ;

    k = t.first <= 10 ? Factorial[t.first.longValue()]: last_non_zero_digit(t.first) ;
    ret = k * bFactorial * p2 ;
    while (ret && ret % 10 == 0) ret/=10;

#ifdef AJAVA_DEBUG
    cout << n << " = 5 * " << t.first << " + " << t.second << "\n";
    cout << " 2^" << t.first << "(" << p2 << ") * " << t.first << "!("<< k <<") * " << t.second << "! ("<< bFactorial <<")\n";
#endif
