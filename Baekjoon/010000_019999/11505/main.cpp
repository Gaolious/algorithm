...
class SegTree {
    ...
    void _build(int idx, int s, int e) {
        ...
        for ( int i = this->size + this->N - 1 ; i > 1 ; i--)
            Tree[i/2] = ( Tree[i/2] * Tree[i] ) % M ;
    }

    void _update_lazy(int idx, int s, int e) {
        if (this->Lazy[idx] != 0) {
            this->Tree[idx] = ( this->Tree[idx] + mMUL(this->Lazy[idx], (T)(e-s+1), M) ) % M ;
            if (s != e ) {
    ...
    void _update(int idx, int s, int e, int l, int r, T diff ) {
        ...
        if (l <= s && e <= r )
        {
            this->Tree[idx] = ( this->Tree[idx] + mMUL(diff, (T)(e - s + 1), M) ) % M ;
            if ( s != e ) {
            ...
            this->Tree[idx] = this->Tree[idx*2] * this->Tree[idx*2+1] % M;
        }
    ...
    T _query(int idx, int s, int e, int l, int r ) {
        ...
        int m = ( s + e ) / 2 ;
        return this->_query(idx * 2, s, m, l, r) * this->_query(idx * 2 + 1, m + 1, e, l, r) % M;
    }
...
void process() {
    ...
    while ( M -- ) {
        ...
        if ( a == 1 ) {
            b--;
            tree.update(b, b, c - tree.data(b));
            tree.dump();
        } else {
            b--;
            c--;
            fio.write_IntLn(tree.query(b, c));
        }
    }
}
