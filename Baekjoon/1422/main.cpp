...
bool comp_R(const int a, const int b) {
    return a > b ;
}
...
int comp_S(const void *_a, const void *_b) {
    const NUMBER *a = (const NUMBER *)_a ;
    const NUMBER *b = (const NUMBER *)_b ;
    int maxN = a->len + b->len ;
    int i;

    for ( i = 0 ; i < maxN ; i ++ ) {
        if ( a->str[ i % a->len ] < b->str[ i % b->len] )
            return 1 ;
        else if ( a->str[ i % a->len ] > b->str[ i % b->len] )
            return 0;
    }
    return 0 ;
}
void process() {
    ...
    for ( i = 0 ; i < K ; i ++ ){
        fio.Int(S[i].num);
        maxN = max(maxN, S[i].num);
        sprintf(S[i].str, "%d", S[i].num);
        S[i].len = strlen(S[i].str);
    }
    qsort(S, K, sizeof(NUMBER), comp_S);

    for ( i = 0 ; i < K ; i ++ ) {
        fio.write_Str(S[i].str);
        if (S[i].num == maxN && N > K ) {
            for ( j = K ; j < N ; j ++)
                fio.write_Str(S[i].str);
            N = K ;
