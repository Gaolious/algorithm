...
void process(vector<Line> &box, Line &l) {
    bool isCross, isCollinear, isOverlap, isTouchBothEndPoint, isTouchSingleEndPoint;
    Line overlapLine;
    int cnt = 0 ;
    vector<Pt> ans;
    for (auto boxline : box) {
        isCross = segmentsIntersect(
                boxline, l,
                overlapLine, isTouchBothEndPoint, isTouchSingleEndPoint, isCollinear, isOverlap
        );
        if (!isCross) continue;
        if ( isOverlap ) {
            cout << 4 << '\n';
