...
struct SegmentTree {
	...
	ll query(int l, int r, int k) {
		...
		for ( l += base, r+=base ; l <= r ; l/=2, r/=2 ) {
			if ( l % 2 == 1 ) {
				ret += T[l].size() - (upper_bound(T[l].begin(), T[l].end(), k) - T[l].begin());
				l++;
			}
			if ( r % 2 == 0 ) {
				ret += T[r].size() - (upper_bound(T[r].begin(), T[r].end(), k) - T[r].begin());
				r--;
			}
		}
		return ret;
