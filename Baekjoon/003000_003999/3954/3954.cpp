...
struct Data {
	...
	void runCommand() {
		...
		memIndex = 0;
		string::iterator strPointer = str.begin();
		stack<pii> currLoop;
		int loop = -1;

		for (i = 0, iter = 0 ; i < Sc && iter < 100'000'000 ; i++, iter ++ ) {
			switch (command[i]) {
				case '-':
					mem[ memIndex ] --;
				break;
				...
				break;
				case '<':
					memIndex = ( memIndex + mem.size() - 1 ) % mem.size();
				break;
				case '>':
					memIndex = ( memIndex + mem.size() + 1 ) % mem.size();
				break;
				case '[':
					if ( mem[memIndex] == 0 ) i = LoopPair[i];
					else currLoop.emplace(i, LoopPair[i]);
				break;
				case ']':
					if ( mem[memIndex] != 0 ) {
						if ( iter > 50'000'000 ) loop = max(loop, i);
						i = LoopPair[i];
					}
					else {
						currLoop.pop();
				...
				case '.':
					break;
				case ',':
					if ( strPointer == str.end())
						mem[memIndex] = 255;
					else
						mem[memIndex] = (int)(*strPointer++);
				break;
			}
		}

		if ( loop >= 0 ) {
			cout << "Loops " << LoopPair[loop] << ' ' << loop << '\n';
		}
		else
			cout << "Terminates\n";
	}
