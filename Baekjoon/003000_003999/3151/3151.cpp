...
ll Count(vector<int> &A, int left, int right, int M) {
    // A는 오름차순 정렬
    // left ~ right 사이에 2개의 수 합이 M인 개수 : two pointer O(N)
    ll ret = 0, lcnt, rcnt, k;
    while ( left < right ) {
        if ( A[left] + A[right] > M ) right--;
        else if ( A[left] + A[right] < M ) left++;
        else if ( A[left] == A[right] ) {
            // -4 2 2 2 2 2  -> 숫자 2 5개중 2개 선택 // 5*4 / 2
            ret += (ll)(right-left) * (right-left+1) / 2;
            break;
        }
        else {
            // -4 1 1 1 3 3  -> lcnt=3, rcnt=2 // 총 6가지
            for (lcnt = 1 ; left < right && A[left] == A[left+1] ; lcnt++, left++ );
            for (rcnt = 1 ; left < right && A[right] == A[right-1] ; rcnt++, right-- );
            ret += rcnt * lcnt;
            left++;
            right--;
        }
