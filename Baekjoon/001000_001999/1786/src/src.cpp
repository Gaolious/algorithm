...
void BoyerMoore_BadCharacterRule(char T[], int lenT, char P[], int lenP)
{
    int idxT, idxP ;
    int offsetP ;
    int i, j, k ;
    ...
    for ( idxP = 0 ; idxP < lenP ; idxP ++ )
        F[P[idxP]] = idxP ;

    for ( offsetP = 0, idxT = 0 , idxP = lenP - 1 ; ; )
    {
        if ( P[offsetP + idxP] == T[idxP] ) idxP --;
        else 
        {
            if ( F[T[idxP]] >= 0 ) offsetP ;
            else offsetP += lenP ;
        }
    }
}
