...
pair<ll, ll> F(ll len) {
    // pair<longest length, first pos>
    ll i, cnt=0, prev;
    int N = A.size() ;

    for ( i = N-1, prev = 0 ; i >= 0 ; i -- ) {
        if ( prev + A[i] > len ) {
            prev = 0;
            cnt++;
        }
...
void process() {
    ...
    l = 1, r = L, ans = L ;
    for ( i = 0 ; i < A.size() ; i ++ ) l = max(l, A[i]);

    while ( l <= r ) {
        len = (l + r) / 2;
        auto [count, first_pos ] = F(len);

        if ( count <= C ) {
            ans = len, pos = first_pos;
            r = len - 1;
        }
        else
            l = len + 1;
