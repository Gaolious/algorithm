...
bool binary_search(int left, int right, int val) {
    ...
    while ( l <= r ) {
        m = (l+r)/2;
        if ( A[m] == val ) return true ; // 찾으면
        if ( A[m] < val ) l = m + 1; // 우쪽이면,
        else if ( val < A[m] ) r = m -1 ; // 좌측이면
    }
    return false;
}
int lower_bound(int left, int right, int K) {
    // K 이상이면서, 가장 작은 index 반환

    int l, r, m, ret = 0 ;
    l = left, r = right;
    while ( l <= r ) {
        m = (l+r)/2;
        // A[l] ~ A[m] <= K 모두 해가 될 수 있을 때
        if ( A[m]  <= K ) {
            ret = m;
            l = m + 1;
        }
...
void process() {
    ...
    cout << lower_bound(0, A.size()-1, 5) << '\n';
    cout << upper_bound(0, A.size()-1, 5) << '\n';
    cin >> M ;
    while ( M-- ) {
        cin >> n ;
        if (binary_search(0, A.size()-1, n)) cout << "1\n";
        else cout << "0\n";
    }
}
