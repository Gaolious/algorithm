...
int D[MaxN+3][MaxN+3][MaxN+3];
#define mod(x, y) do { (x) =  ( (x) + (y) ) % MOD ; } while (false)
int F(int N, int M, int K) {
	...
	if ( ret < 0 ) {
		ret = F(N - 1, M, K);
		ret += ((ll)F(N - 1, M - 1, K - 1) * M ) % MOD;
		ret += ((ll)F(N - 1, M - 2, K - 2) * M * (M - 1) / 2) % MOD;
		ret += ((ll)F(N - 2, M - 1, K - 2) * M * (N - 1) ) % MOD;
		ret %= MOD;
	}
	return ret;
}
