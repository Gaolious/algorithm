...
const int columnCount = BASE_4 * 4;
int sparseMatrix[BASE_4 * BASE_2][4];
int rows;
point points[BASE_4 * BASE_2];

const int NodeCacheCount = columnCount + 1 + BASE_4 * BASE_2 * 4;
node NodeCache[NodeCacheCount]{};
int NodeCacheIndex = 0;
char table[BASE_2][BASE_2];
bool horizontal[BASE_2][BASE_2+1];
bool vertical[BASE_2][BASE_2+1];
bool boxed[BASE][BASE][BASE_2+1];

void init() {
    NodeCacheIndex = 0;

    for (int i = 0; i < columnCount; i++) {
        columns[i] = NodeCache + NodeCacheIndex++;
        columns[i]->size = 0;
    }
    ...
    for (int row = 0; row < rows; row++) {
        ...
        for (int col : sparseMatrix[row]) {
            current = NodeCache + NodeCacheIndex++;
            current->row = row;
...
bool sudoku(char s[]) {
    ...
    memset(boxed, 0, sizeof(vertical));
    ...
    for (int y = 0; y < BASE_2; y++) {
        for (int x = 0; x < BASE_2; x++) {
            ...
            for (int i = 0; i < BASE_2; i++) {
                if (horizontal[x][i] || vertical[y][i] || boxed[y / BASE][x / BASE][i])
                    continue;
    ...
    if (ret) {
        ...
        for (int y = 0; y < BASE_2; y++)
            for (int x = 0; x < BASE_2; x++) {
                if (table[y][x] == EMPTY)
                    return false;
                if ( horizontal[x][table[y][x]] ) return false ;
                horizontal[x][table[y][x]] = true ;
                if ( vertical[y][table[y][x]] ) return false ;
                vertical[y][table[y][x]] = true ;
                if ( boxed[y / BASE][x / BASE][table[y][x]] ) return false ;
                boxed[y / BASE][x / BASE][table[y][x]] = true ;
            }
        for (int i = 0 ; i < BASE_2 ; i ++) {
            for (int x = 0; x < BASE_2; x++)
                if ( !horizontal[x][i] ) return false ;
            for (int y = 0 ; y < BASE_2 ; y ++)
                if ( !vertical[y][i] ) return false ;
            for (int y = 0 ; y < BASE ; y ++)
                for (int x = 0; x < BASE; x++)
                    if ( !boxed[y][x][i] ) return false;
        }
    }
    return ret;
}
...
int main()
{
    ...
    char M[BASE_2*BASE_2+1]{};
    bool is_success = true ;
    for ( m=0 ; m < BASE_2*BASE_2 ; m ++)
        M[m] = CHAR_EMPTY;

    for ( m=0 ; m < BASE_2*BASE_2 ; m ++) {
        cin >> y >> x >> v;
        y--; x--;
        if (M[ y * BASE_2 + x] != CHAR_EMPTY )
            is_success = false;
