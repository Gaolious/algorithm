...
void process() {
    ...
    cin >> N ;
    vector<pii> A(N+1);
    vector<int> use(N+1), earn(N+1), S(N+1);
    int maxUse=0, maxUseDay;
    for ( i = 1 ; i <= N ; i ++ ) {
        cin >> A[i].first  >> A[i].second;
        if (  A[i].first - A[i].second > maxUse ) {
            maxUse = A[i].first -A[i].second;
            maxUseDay = i;
        }
        use[i] = A[i].first + use[i-1];
        earn[i] = A[i].second + earn[i-1];
        S[i] = -use[i] + earn[i];
    }

    if ( use[N] < earn[N] ) cout << "PLUSSIS\n";
    else if ( use[N] > earn[N] ) cout << "MIINUSES\n";
    else cout << "NULLIS\n";

    if ( maxUse > 0 )
        cout << maxUseDay << ' ' << maxUse << '\n';
    else
        cout << "0 0\n";
    ...
    for ( s = i = 1 ; i <= N ; i ++ ) {
        ...
        if ( S[i] - S[s-1] < maxUseRange ) {
            maxUseRange = S[i] - S[s-1];
            useRange = {s, i};
        }
    }
    if ( maxUseRange < 0 ) {
        cout << useRange.first << ' ' << useRange.second << ' ' << -maxUseRange << '\n';
    }
    else {
        cout << "0 0 0\n";
    }
