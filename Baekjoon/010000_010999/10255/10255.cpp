...
struct ConvexHull {
    typedef enum {NO_CROSS, CROSS, OVERLAP} CROSS_TYPE;

    vector<Point> ch;
    void build(vector<Point> &A, bool remove_line = true) {
        ch.clear();
    ...
    CROSS_TYPE is_cross(Line &l, vector<Point> &ret ) {
        // 0 : no cross
        // 1 : cross only one point
        // 2 : cross two points
        // 4 : overlapped line
        ret.clear();

        for( int i = 1 ; i <= ch.size() ; i ++ ) {
            Line line( ch[i-1], ch[i%ch.size()] );
            Point p;
            if (line.is_cross(l, p) ) {
                ret.push_back(p);
            }
        }
    }
