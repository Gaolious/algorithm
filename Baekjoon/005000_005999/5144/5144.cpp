...
void p2(){
    ...
    vector<vector<int>> requiredCount(N);
    for ( i = 0 ; i < N ; i ++ ) {
        cin >> Species[i];
        requiredCount[i].resize(i);
        for (j = 0; j < i; j++)
            cin >> requiredCount[i][j];
    }
    for ( i = 1 ; i < N ; i ++ ) {
        cnt = Species[i] ;
        for ( j = 0 ; j < i ; j ++ ) {
            if ( requiredCount[i][j] < 1 ) continue;
            cnt = min(cnt, Species[j]/requiredCount[i][j]);
        }
        Species[i] = cnt;
        for ( j = 0 ; j < i ; j ++ )
            Species[j] -= cnt * requiredCount[i][j];
    }
    for ( i = 0 ; i < N ; i ++ )
        cout << Species[i] << '\n';
}
