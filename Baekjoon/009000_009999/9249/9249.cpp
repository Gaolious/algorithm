...
#define BETWEEN(l, v, r) ( (l) <= (v) && (v) < (r) )
	for(i = 1; i < s3.size() ; i++) {
		if (
			(BETWEEN(0, sa[i-1] + lcp[i] - 1, s1.size()) && BETWEEN(s1.size(), sa[i] + lcp[i] - 1, s3.size())) ||
			(BETWEEN(0, sa[i] + lcp[i] - 1, s1.size()) && BETWEEN(s1.size(), sa[i-1] + lcp[i] - 1, s3.size()))
		)
		{
			if ( ans_index < 0 || lcp[ans_index] < lcp[i] ) {
				ans_index = i;
			}
		}
	}
	...
	cout << lcp[ans_index] << '\n';
	cout << s3.substr(sa[ans_index], lcp[ans_index]);
	cout << '\n';
}
