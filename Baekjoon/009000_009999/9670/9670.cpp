...
int getPrice(double w, double h){
    int price;
    double maxUsage = -1, ratio = -1;
    int newW, newH ;
    for (int i = 0 ; i < 2 ; i ++ ) {
        for (auto &d: D) {
            if ( (h / w) > (d.resolution.first/d.resolution.second) ) {
                newW = d.resolution.first ;
                newH = w * d.resolution.first / h;
            }
            else {
                newW = h * d.resolution.second / w ;
                newH = d.resolution.second;
            }
            ratio = (newW / d.resolution.first) * (newH / d.resolution.second);

            if (maxUsage < ratio || ( abs(maxUsage - ratio) < 1e-9 && price > d.price )) {
                maxUsage = ratio;
                price = d.price;
            }
        }
