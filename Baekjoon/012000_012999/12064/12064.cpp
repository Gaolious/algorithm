...
vector<ll> sliding(vector<ll> &num, ll k) {
    deque<ll> Q;
    vector<ll> ret;
    for ( int i = 0 ; i < num.size() ; i ++ ) {
        ...
        while ( !Q.empty() && num[ Q.back() ] < num[ i ] )
            Q.pop_back();
        Q.push_back(i);
        if ( i >= k - 1 ) ret.push_back(num[Q.front()]);
    }
    return ret;
}
void process() {
    ...
    for ( caseNo = 1 ; caseNo <= T ; caseNo ++ ) {
        ...
        for ( i = 0 ; i < N ; i ++ )
            matrix[i] = sliding(M[i], K);
        for ( j = 0 ; j <= N-K ; j ++ ) {
            vector<ll> col;
            for ( i = 0 ; i < N ; i ++) col.push_back(matrix[i][j]);
            for (auto n: sliding(col, K) ) s += n;
        }
        cout << "Case #" << caseNo << ": " << s << '\n';
    }
}
