...
enum STONE { KING, QUEEN, ROCK, BISHOP, KNIGHT, PAWN, EMPTY };
...
void process() {
    ...
    for ( i = 0 ; i < 8 ; i ++ ) {
        for ( j = 0 ; j < 8 ; j ++ ) {
            STONE stone = getStone(Board[i][ j * 4 + 2]);
//            TURN turn = Board[i][j*4+1] == ':' ? BLACK : WHITE ;
            TURN turn = 'A' <= Board[i][j*4+2] && Board[i][j*4+2] <= 'Z' ? WHITE : BLACK;
            if ( stone == EMPTY ) continue;
            A[turn].push_back({8-i, j+1, stone, Board[i][ j * 4 + 2], turn});
        }
    }
