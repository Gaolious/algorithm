...
int sum( int sy, int sx, int ey, int ex) {
    // (sy,sx) ~ (ey,ex) / 1-index
    return S[ey][ex] - S[sy-1][ex] - S[ey][sx-1] + S[sy-1][sx-1];
}

void process() {
    int i, j, K, sy, sx, ey, ex;
    cin >> Y >> X ;
    A.resize(Y+1, vector<int>(X+1));
    S.resize(Y+1, vector<int>(X+1));
