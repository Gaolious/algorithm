...
void process() {
    LazySegmentTree lstCnt;

	int N, a, b, cnt ;
	vector<pii> A;
	vector<int> FlowCount(MaxN+1);
	lstCnt.init(MaxN+1);
	...
	while (N-- ) {
		...
		cnt = 0;
		for (auto k: {a, b}) {
			auto lstSum = lstCnt.query(1, 1, MaxN, k, k).sum;
			if ( FlowCount[k] < lstSum ) {
				cnt += lstSum - FlowCount[k];
				FlowCount[k] = lstSum;
			}
		}
		cout << cnt << '\n';
		lstCnt.Update(1, 1, MaxN, a+1, b-1, 1);
	}
}
