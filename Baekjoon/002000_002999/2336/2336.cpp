...
struct SegmentTree {
	...
	void update(int idx, int val) {
		int i;
		for ( T[idx += base] = val ; idx > 1 ; idx /= 2 ) {
			T[idx/2] = Merge(T[idx], T[idx^1]);
		}
	}
	int query(int l, int r) {
		int ret = INF;
		for ( l += base, r+=base ; l <= r ; l/=2, r/=2 ) {
			if ( l % 2 == 1 ) ret = Merge(ret, T[l++]);
			if ( r % 2 == 0 ) ret = Merge(ret, T[r--]);
		}
		return ret;
	}
};
...
void process() {
	...
	for ( i = 1 ; i <= N ; i ++ ) {
		auto ret = st.query(1, student[i].rank[1]-1);
		if ( ret < student[i].rank[2] )
			ans[i] = false;
		st.update(student[i].rank[1], student[i].rank[2]);
	}
	int cnt = 0;
	for ( i = 1 ; i <= N ; i ++ )
		cnt += ans[i];
