...
int recur(int s, int visits, int e_mask, int depth = 0 )
{
    if ( visits == ( ( 1 << N ) - 1 ) )
        return 0;
    else if ( e_mask & visits )
        return INF ;

    //printf("%*s Visit s(%d), visits(%s)\n", depth*4, " ", s, toBinary(visits) );
    ...
    if ( DP[ s ][ visits ] < 0 )
    {
        ...
        for ( i = 0 ; i < N ; i ++ )
        {
            ...
            if ( cost[s][i] == 0 ) continue ;

            ret = min( ret, recur( i, visits | ( 1 << i ) , e_mask, depth + 1 ) + cost[s][i] );

//            printf("%*s Update s(%d), visits(%s) : %d\n", depth*4, " ", s, toBinary(visits), ret );
        }
