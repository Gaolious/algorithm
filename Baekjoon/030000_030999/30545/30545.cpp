...
struct Pos {
    ...
    int order{};
    Pos() {
        offset = -1, cycle_length = -1, order = -1, c = -1;
    }
    bool operator <( Pos &o ) {
        if ( cycle_length != o.cycle_length ) return cycle_length > o.cycle_length;
...
void process() {
    ...
    for ( i = 0 ; i < 26 ; i ++ ) {
        ...
        P[ i ].c = (char)(i+'a');
        P[ i ].cycle_length = -1;
        for ( j = 1 ; j < Index[i].size() ; j ++ )
            P[ i ].cycle_length = max(P[ i ].cycle_length, Index[i][j].first - Index[i][j-1].first);

        for ( j = 1 ; j < Index[i].size() ; j ++ ) {
            if (Index[i][j].first - Index[i][j - 1].first == P[i].cycle_length) {
                P[i].order = Index[i][j].second - Index[i][j - 1].second ;
                P[i].offset = j;
                break;
            }
        }
...
//    for ( i = 0 ; i < 26 ; i ++ ) {
//        if ( P[i].c < 0 ) continue;
//        cout << P[i].c << " : " << P[i].offset << ", " << P[i].cycle_length << ", " << P[i].order << '\n';
//    }
    cout << P[0].c;
}
int main()
