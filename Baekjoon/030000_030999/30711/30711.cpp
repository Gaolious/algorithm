...
void process() {
    ...
    // N : 대회 참가자 수,
    // Rank : 결선 진출 기준이 되는 참가자 점수 합계 순위
    // C : 짧은 예선에서 얻을 수 있는 최대 점수
    cin >> N >> Rank >> C;

    vector<ll> A(N), Origin(N);
    for ( i = 0 ; i < N ; i ++ ) {
        cin >> A[ i ] ;
        Origin[ i ] = A[ i ];
    }
    ...
    for ( auto n: Origin ) {
        int a = lower_bound(A.begin(), A.end(), n + C, greater<>()) - A.begin();
        int b = lower_bound(A.begin(), A.end(), n - C, greater<>()) - A.begin() - 1;

        if (a >= Rank ) cout << "Fail\n";
        else if ( b < Rank ) cout << "Pass\n";
        else cout << "Short\n";
    }
}
