...
struct _poistion_ {
    ...
    void dump()
    {
        ...
        printf("\n");
        printf("curr => %s\n", this->dumpStr());
        for ( x = -BLOCK_SIZE ; x <= BLOCK_SIZE/2 ; x ++ )
            printf("%c", abs(x) % 10 == 0 ? '+' : '0' + abs(x)%10 );
        printf("\n");
        for ( y = BLOCK_SIZE ; y >= -BLOCK_SIZE/2 ; y -- )
        {
            for ( x = -BLOCK_SIZE ; x <= BLOCK_SIZE/2 ; x ++ )
            {
...
{
    char command [100]={0,};
    while ( fgets(command, sizeof(command)-1, stdin) && strncmp(command, "END", 3) != 0 )
    {
        POSITION pos ;
        char *p = command;
        ...
        while ( fgets(command, sizeof(command)-1, stdin) && strncmp(command, "STOP", 3) != 0 )
        {
            p = command;
        ...
        if ( pos.is_stopable() && next.valid_position())
            printf("%s\n", pos.dumpStr());
        else
