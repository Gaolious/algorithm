...
func P50() uint64 {
	var Primes = utils.GetUint64Primes(1000000)
	var S = make([]uint64, len(Primes)+1)
	var i, maxT int
	var ret uint64
	var has = func(n uint64, R int) int {
		l := 1
		r := maxT + l
		for r - l >= maxT && r <= R {
			s := S[r] - S[l-1]
...
func P51() uint64 {
	...
	const MaxN = uint64(9_999_999)
	var Primes = utils.GetUint64Primes(MaxN)
	var check [MaxN+1]byte
	var recur func(idx, remain, l, r int, arr []int) uint64

	for _, p := range Primes {
		check[p] = 1
	}
	var FindPrimeWithReplace = func(l, r int, arr []int) uint64 {
		type DATA struct {
			cnt int
			primes []uint64
		}
		var D = make(map[[10]byte]*DATA)
		var n10, v uint64
		var j int
		...
		for i := l ; i <= r ; i ++ {
			...
			var key [10]byte
			for n10, j = uint64(1), len(arr)-1; j >= 0; j-- {
				if arr[j] == 1 {
					key[j] = '*'
				} else {
					v = (p / n10) % 10
			...
			D[key] = &DATA{cnt: 0, primes: make([]uint64, 0)}
			newPrime := uint64(0)

			for replaceN := uint64(0); replaceN <= 9; replaceN++ {
				if replaceN == 0 && arr[0] == '1' {
					continue
				}
				...
				(D[key]).cnt++
				(D[key]).primes = append((D[key]).primes, newPrime)
			}
		}
		for k, v := range D {
			if v.cnt == 8 {
	...
	recur = func(idx, remain, l, r int, arr []int) uint64 {
		if remain == 0 {
			fmt.Println(arr)
			ret := FindPrimeWithReplace(l, r, arr)
			if ret > 0 {
				return ret
			}
			return 0
		}
		if idx >= len(arr) || len(arr) - idx < remain {
			return 0
		}
		arr[idx] = 1
		ret := recur(idx+1, remain - 1, l, r, arr)
		arr[idx] = 0
		if ret > 0 { return ret }

		ret = recur(idx+1, remain, l, r, arr)
		return ret
	}
	...
	for N = 1 ; N < MaxN+1 ; N *= 10 {
		cnt++
		l = r + 1

		for r = l ; r < len(Primes) - 1 && Primes[r+1] < N * 10 ; r ++ {}
		arr := make([]int, cnt)

		for k:= 1 ; k < cnt; k ++ {
			ret := recur(0, k, l, r, arr)
			if ret > 0 {
				return ret
			}
		}
...
`
	...
	var makeM = func(card [4][15]bool)int{
		var A = make([]int, 0)
		for i:=0 ; i < 4 ; i ++ {
			for j := 1 ; j <= 14 ; j ++ {
				if card[i][j] {
	...
	var makeN = func(a,b,c,d,e int) int {
		var A = []int {a, b, c, d, e}
		sort.Sort(sort.Reverse(sort.IntSlice(A)))
	...
	var GetScore = func(card [4][15]bool) (int, int, int) {
		var i, j, k int

		for i = Club ; i <= Spade ; i ++ {
			if card[i][10] && card[i][11] && card[i][12] && card[i][13] && card[i][14] {
		...
		for i = Club ; i <= Spade ; i ++ {
			for j = 1 ; j <= 14-5 ; j ++ {
				if card[i][j] && card[i][j+1] && card[i][j+2] && card[i][j+4] && card[i][j+5] {
					return StraightFlush, j+4, 0
				}
			}
		}

		for j = 1 ; j<= 14 ; j ++ {
			if card[0][j] && card[1][j] && card[2][j] && card[3][j] {
				card[0][j] = false
				card[1][j] = false
				card[2][j] = false
				card[3][j] = false
				return FourOfaKind, j, makeM(card)
			}
		}
		...
		for j = 1 ; j<= 14 ; j ++ {
			...
			if cnt3 == 3 {
				for k = 1 ; k <= 14 ; k ++ {
					...
					if cnt2 == 2 {
						return FullHouse, makeN(j,j,j,k,k), 0
					}
				}
			}
		}
		...
		for i = 0 ; i < 4 ; i ++ {
			...
			if cnt == 5 {
				return Flush, makeN(arr[0], arr[1], arr[2], arr[3], arr[4]), 0
			}
		}
		...
		for j = 1 ; j <= 14-4 ; j ++ {
			...
			for k = 0 ; k < 5 ; k ++ {
				f1 = false
				for i = 0 ; i < 4 ; i ++ {
					if card[i][j+k] { f1 = true; break }
				}
				if !f1 { f2 = false; break; }
			}
			if f2 {
				return Straight, makeN(j, j+1, j+2, j+3, j+4), 0
			}
		}
		...
		for j = 1 ; j<= 14 ; j ++ {
			...
			if cnt3 == 3 {
				...
				return ThreeOfaKind, makeN(j, j, j, 0, 0), makeM(card)
			}
		}
			...
			if cnt2 == 2 {
				for k = j + 1 ; k <= 14 ; k ++ {
					...
					if cnt22 == 2 {
						...
						return TwoPairs, makeN(j, j, k, k, 0), makeM(card)
					}
				}
			}
		}
	...
	for _, line := range strings.Split(data, "\n") {
		...
		N['T'], N['J'], N['Q'], N['K'], N['A'] = 10, 11, 12, 13, 14
		C['C'], C['D'], C['H'], C['S'] = 0, 1, 2, 3
		...
		fmt.Println(line)
		a1, b1, c1 := GetScore(p[0])
		a2, b2, c2 := GetScore(p[1])
		fmt.Println("p1=", a1,b1, "p2=", a2, b2)
		if a1 > a2 { ret++; continue }
		if a1 < a2 {
			continue
		}
		if b1 > b2 { ret++; continue }
		if b1 < b2 {
			continue
		}
		if c1 > c2 { ret ++; continue }
		if c1 < c2 { continue }
		fmt.Printf("error.\n")
	}
	return ret
...
func P55() int {
	...
	for i = 1 ; i < 10000 ; i ++ {
		...
		n := utils.NtoByte(i)
		for j = 0 ; j < 50 ; j ++ {
			r := utils.InverseByte(n)
			s := utils.SumTwoByteInteger(n, r)
			if utils.IsPalindromeByte(s) {
				found = true
				break
			}
			n = s
...
func P56() int {
	...
	for i = 2 ; i < 100 ; i ++ {
		...
		b := big.NewInt( 1 )
		for j = 1 ; j < 100 ; j ++ {
			c := new(big.Int).Mul(b, a)
			b = c
			s = 0
			for _, t := range c.Text(10) {
				s += int(t - '0')
			}
			if s > ret { ret = s }
		}
...
func P58() int {
	var i, size, j, n, total, nPrime int
	...
	for i = 'a' ; i <= 'z' ; i ++ {
		...
		for j = 'a' ; j <= 'z' ; j ++ {
			...
			for k = 'a' ; k <= 'z' ; k ++ {
				...
				decrypt()
				if isPossible() {
					for _, v := range decrypted_string {
						ret += int(v)
					}
					return ret
				}
